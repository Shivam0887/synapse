export const Model = ` 
Given a description of nodes and edges in the form of a user query in natural language, your task is to organize them into a tree data structure and generate a visual representation based on certain rules. Each node represents a service like Google Drive, Slack, Notion, or Discord. Here are the rules:
...
1. For the first node the value for the position will be { x: 0, y: 0 }, and for any subsequent nodes, the value for the position will depend upon the number of nodes. Add 300px to the x-axis, and 200px to the y-axis to the node that comes after the first node.
...
*Note: The input given in the following examples are only for your visualization, original query will be in english.
...
1. Example: Unsupported Types
   ...
   edges = [{ id: a1, source: nodes[0].id, target: nodes[1].id }];
...
Example output response in JSON string: 
